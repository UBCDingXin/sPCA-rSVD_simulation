...
# setwd("C:/Users/Xin/ownCloud/Research/STAT548/Fourth paper_Gaby/Simulation")
...
output_cv = array(0, c(3, 8))
output_cv[1,] = c(median(err2_soft_cv[,1]), colMeans(err2_soft_cv[,2:3]) * 100, mean(err2_soft_cv[,1]),
                  median(err2_soft_cv[,4]), colMeans(err2_soft_cv[,5:6]) * 100, mean(err2_soft_cv[,4]))
output_cv[2,] = c(median(err2_hard_cv[,1]), colMeans(err2_hard_cv[,2:3]) * 100, mean(err2_hard_cv[,1]),
                  median(err2_hard_cv[,4]), colMeans(err2_hard_cv[,5:6]) * 100, mean(err2_hard_cv[,4]))
output_cv[3,] = c(median(err2_SCAD_cv[,1]), colMeans(err2_SCAD_cv[,2:3]) * 100, mean(err2_SCAD_cv[,1]),
                  median(err2_SCAD_cv[,4]), colMeans(err2_SCAD_cv[,5:6]) * 100, mean(err2_SCAD_cv[,4]))

output_sel_varnum = array(0, c(3,2))
output_sel_varnum[1,1] = median(sel_varnum_soft[,1])
output_sel_varnum[1,2] = median(sel_varnum_soft[,2])
output_sel_varnum[2,1] = median(sel_varnum_hard[,1])
output_sel_varnum[2,2] = median(sel_varnum_hard[,2])
output_sel_varnum[3,1] = median(sel_varnum_SCAD[,1])
output_sel_varnum[3,2] = median(sel_varnum_SCAD[,2])
...
wb <- XLConnect::loadWorkbook("tmp2.xlsx", create = TRUE)
sheetname="Sheet1"
XLConnect::writeWorksheet(wb,round(output_cv,digits=2),sheetname,startRow = 1, startCol = 1, header = FALSE)
XLConnect::saveWorkbook(wb)
